...
/// <summary>Handles <see cref="AddWorkPatternCommand"/> by loading the aggregate, invoking domain addition, and committing the unit of work.</summary>
public sealed class AddWorkPatternCommandHandler : IRequestHandler<AddWorkPatternCommand, Result<Guid>>
{
    ...
    /// <summary>Initializes a new instance of the <see cref="AddWorkPatternCommandHandler"/> class.</summary>
    ...
    public AddWorkPatternCommandHandler(
    ...
    /// <returns>A result containing the new pattern's identifier, or a business error.</returns>
    public async Task<Result<Guid>> Handle(AddWorkPatternCommand request, CancellationToken cancellationToken)
    {
        ...
        var result = calendar.AddWorkPattern(request.StartDate, request.EndDate, request.WeeklySchedule, _dateTimeProvider);
