...
/// <summary>
/// Handles <see cref="RegisterColorCommand"/>. Scope is resolved
/// directly from HoldingId — no cross-module lookup needed, since
/// Color is itself Holding-scoped (chat, 2026-08-30).
/// </summary>
public sealed class RegisterColorCommandHandler : IRequestHandler<RegisterColorCommand, Result<Guid>>
{
    ...
    /// <returns>A <see cref="Result{Guid}"/> containing the new Color's identifier, or a validation/authorization error.</returns>
    public async Task<Result<Guid>> Handle(RegisterColorCommand request, CancellationToken cancellationToken)
    {
        if (_currentUserService.UserId is not { } userId)
        {
            return Result.Failure<Guid>(global::Configuration.Domain.ColorErrors.NotAuthorized());
        }
        ...
        var result = global::Configuration.Domain.Color.Register(request.HoldingId, request.Name, _dateTimeProvider);
